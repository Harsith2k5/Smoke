...
// Label for pollution info (temp, wind, AQI)
function createInfoLabel(text) {
  const canvas = document.createElement('canvas');
  canvas.width = 512;
  canvas.height = 256;
  const ctx = canvas.getContext('2d');
  ctx.fillStyle = '#D8BFD8'; // light purple
...
for (let i = 0; i < NO2_COUNT; i++) {
  ...
  const textLabel = createLabel(NO2_LABEL);
  textLabel.position.set(0, 0, 0.1); // just above the plane
  plane.add(textLabel);
